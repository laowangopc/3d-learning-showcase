...
import { learningRouter, learningNativeGuard } from "../learning/index.js";
...
export default async function createServer(locals:AppParameters) :Promise<express.Application>{
  ...
  app.use(learningNativeGuard);
  app.use('/learn', (req,res,next) => {
    // Voyager renders its bundled SVG controls through <object>; uploaded files
    // are never allowed here. Use an exact first-party distribution directory.
    res.removeHeader('Content-Security-Policy-Report-Only');
    res.set('Content-Security-Policy', `default-src 'self'; script-src 'self' 'wasm-unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; font-src 'self' data:; connect-src 'self' data: blob:; worker-src 'self' blob:; frame-src 'self'; object-src ${getHost(req).origin}/dist/images/; base-uri 'self'; form-action 'self'; frame-ancestors 'self'`);
    next();
  });
  app.use("/learn/api", learningRouter);
  app.use("/learn", express.static("/app/learning", { index: "index.html" }));
