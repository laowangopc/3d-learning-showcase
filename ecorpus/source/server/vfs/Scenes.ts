...
export default abstract class ScenesVfs extends BaseVfs{
  ...
  /** Serialize learning metadata revisions on a stable row inside withTransaction. */
  async lockLearningScene(name: string): Promise<void> {
    const row = await this.db.get(`SELECT scene_id FROM scenes WHERE scene_name = $1 AND archived IS NULL FOR UPDATE`, [name]);
...
`;
  ...
  static _validateSceneQuery(q :Readonly<SceneQuery|any>):SceneQuery{
    ...
    if(typeof q.type !== "undefined" && (q.type !== "html" && q.type !== "voyager" && q.type !== "panorama")){
